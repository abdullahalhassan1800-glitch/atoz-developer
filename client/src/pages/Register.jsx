...
export default function Register() {
  ...
  const iconInput = "w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-700 text-sm font-light placeholder:text-slate-300 outline-none focus:border-accent focus:ring-2 focus:ring-sky-100 transition-all";
        ...
        <ScaleIn>
          <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-medium">
            <div className="text-center mb-7">
              ...
              <p className="text-sm text-slate-400 mt-1">Join A TO Z Developer</p>
            </div>
            ...
            <form onSubmit={submit} className="space-y-4">
              <div>
                <label className="text-[10px] font-semibold text-slate-400 tracking-[0.15em] uppercase mb-1.5 block">Full Name</label>
              ...
              <div>
                <label className="text-[10px] font-semibold text-slate-400 tracking-[0.15em] uppercase mb-1.5 block">Phone</label>
                <div className="relative">
                  <FaPhone className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-300 text-xs" />
              ...
              <div>
                <label className="text-[10px] font-semibold text-slate-400 tracking-[0.15em] uppercase mb-1.5 block">I am a</label>
                <div className="grid grid-cols-2 gap-3">
                  {[{ v: 'buyer', l: 'Buyer / Renter' }, { v: 'agent', l: 'Property Agent' }].map((o) => (
                    <motion.button type="button" key={o.v} whileTap={{ scale: 0.97 }} onClick={() => setForm({ ...form, role: o.v })} className={`py-3 rounded-xl text-sm font-medium border transition-all duration-300 ${form.role === o.v ? 'border-accent bg-sky-50 text-accent' : 'border-slate-200 text-slate-400 hover:border-slate-300'}`}>
                      {o.l}
                    </motion.button>
                  ))}
                </div>
            ...
            <div className="text-center mt-6">
              ...
              <p className="text-sm text-slate-400">
                ...
                <Link to="/login" className="text-accent font-medium hover:text-accent-dark transition-colors">Sign in</Link>
              </p>
            </div>
          </div>
        </ScaleIn>
