  ...
  if (!property) return (
    <div className="min-h-screen flex items-center justify-center bg-deep">
      <div className="text-center">
        ...
        <Link to="/properties" className="text-sm font-medium text-accent hover:text-accent-dark transition-colors">Back to Properties</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-deep pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24">
        <FadeIn>
          <Link to="/properties" className="inline-flex items-center gap-2 text-sm font-medium text-accent hover:text-accent-dark transition-colors mb-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-5">
            <FadeIn delay={0.1}>
              <div className="bg-white rounded-2xl overflow-hidden border border-slate-200 shadow-soft">
                <div className="h-72 sm:h-96 md:h-[28rem] relative">
                  ...
                  <div className="absolute top-4 left-4 flex gap-2">
                    ...
                    <span className="px-3 py-1 text-[10px] font-semibold rounded-lg bg-white/90 text-slate-700">
                ...
                {images.length > 1 && (
                  <div className="flex gap-2 p-3 overflow-x-auto">
                    {images.map((i2, i) => (
                      <button key={i} onClick={() => setImg(i)} className={`flex-shrink-0 w-20 h-14 rounded-xl overflow-hidden border-2 transition-all ${img === i ? 'border-accent shadow-medium' : 'border-transparent hover:border-slate-300'}`}>
            ...
            <FadeIn delay={0.2}>
              <div className="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-soft">
                <div className="flex flex-wrap items-start justify-between gap-4 mb-5">
                  <div>
                    <h1 className="text-xl sm:text-2xl font-bold text-slate-800 leading-tight">{property.title}</h1>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-5 bg-slate-50 rounded-xl mb-6 border border-slate-100">
                  {[
                    ...
                    <div key={i} className="text-center">
                      ...
                      <span className="text-sm font-semibold text-slate-800 block capitalize">{item.value}</span>
                    ...
                    <StaggerContainer>
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                        {property.features.map((f, i) => (
                          <StaggerItem key={i}>
                            <div className="flex items-center gap-2 text-sm text-slate-500 p-2.5 bg-slate-50 rounded-xl border border-slate-100">
                ...
                {property.nearbyPlaces?.length > 0 && (
                  <div className="mt-6">
                    <h3 className="font-semibold text-slate-800 mb-3">Nearby Places</h3>
                    <StaggerContainer>
          ...
          <div className="space-y-5">
              ...
              {property.agent && (
                <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-soft">
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-12 h-12 gradient-accent rounded-full flex items-center justify-center text-white font-bold shadow-medium">
                      {property.agent.name?.charAt(0)}
                    </div>
                    <div>
                      <h4 className="font-semibold text-slate-800 text-sm">{property.agent.name}</h4>
                      <p className="text-xs text-slate-400">Property Agent</p>
                    </div>
                  </div>
                  <div className="space-y-1">
                    {property.agent.phone && (
                      <a href={`tel:${property.agent.phone}`} className="flex items-center gap-2.5 text-sm text-slate-500 hover:text-accent p-2.5 rounded-xl hover:bg-slate-50 transition-all">
                        <FaPhone className="text-accent text-xs" /> {property.agent.phone}
                      </a>
                    )}
                    <a href={`mailto:${property.agent.email}`} className="flex items-center gap-2.5 text-sm text-slate-500 hover:text-accent p-2.5 rounded-xl hover:bg-slate-50 transition-all">
            ...
            <FadeIn delay={0.25}>
              <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-soft">
                <h3 className="font-semibold text-slate-800 mb-4">Send Inquiry</h3>
                {sent ? (
                  <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center py-8">
                    <div className="w-12 h-10 gradient-accent rounded-xl flex items-center justify-center mx-auto mb-3 shadow-medium">
                  ...
                  <form onSubmit={send} className="space-y-3">
                    ...
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" className="w-full py-3 gradient-accent text-white text-sm font-semibold rounded-xl shadow-medium hover:shadow-strong transition-shadow">
                      Send Message
                    </motion.button>
                  </form>
                )}
